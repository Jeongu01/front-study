...
const createTree = function (folder, node) {
    ...
    folder.forEach(element => {
        ...
        detailsTag.setAttribute('class', 'detailsTag');
        node.appendChild(detailsTag);
        
        let summaryTag = document.createElement('summary');
        summaryTag.innerHTML = element.name;
        detailsTag.appendChild(summaryTag);

        if(element.folder != null){
            createTree(element.folder, node.querySelector('.detailsTag'));
        }

        element.file.forEach((fileElement) => {
            let liTag = document.createElement('li');
            liTag.innerHTML = fileElement;
            detailsTag.appendChild(liTag);
        });
